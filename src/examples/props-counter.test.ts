...
describe('CounterDisplay', () => {
  it('hydrates initial count', () => {
    ...
    expect(counter.shadowRoot!.textContent).to.contain('The count is: 5.');
    expect(counter.initialCount).to.eq(5);
  });
  ...
  it('hydrates with provided property', () => {
    ...
    expect(counter.shadowRoot!.textContent).to.contain('The count is: 10.'); // Prop bound to DOM.
    expect(counter.initialCount).to.equal(5); // Value read from HTML.
  });
});
...
describe('StateHostCounter', () => {
  it('increments', () => {
    ...
    expect(display.shadowRoot!.textContent).to.contain('The count is: 5.');
    ...
    increment.click();
    expect(display.shadowRoot!.textContent).to.contain('The count is: 6.');
  });
  ...
  it('decrements', () => {
    ...
    decrement.click();
    expect(display.shadowRoot!.textContent).to.contain('The count is: 4.');
  });
});
