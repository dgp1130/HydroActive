...
/**
 * A `Hook` is a function called on `connectedCallback()` and which returns an optional
 * `Disposer` which is invoked on `disconnectedCallback()`. This provides an opportunity
 * to configure logic with cleanup steps to avoid leaking memory. Note that a hook may
 * be executed multiple times if a component is removed and reattached to the DOM.
 */
export type Hook = () => Disposer | void;
...
class Component<Host extends HTMLElement = HTMLElement> {
  ...
  private constructor({ el }: { el: Host }) {
    this.host = el;
    componentInternalStateMap.set(this, { hooks: [] });
  }
  ...
  /**
   * Schedules a hook to be run on the component. The callback is invoked during
   ...
   * operation when the component is not in use. Note that the hook can be called
   ...
   */
  public use(cb: Hook): void {
    // Remember the hook so `connectedCallback()` can invoke it later.
    componentInternalStateMap.get(this)!.hooks.push(cb);
    ...
    const componentState = elementInternalStateMap.get(this.host)!;
    if (componentState.hydrated && this.host.isConnected) {
      const disposer = cb();
      if (disposer) componentState.hookDisposers.push(disposer);
    }
  }
